...
async function exportDay(report: DailyReport, company: Company, date: string, format: 'pdf' | 'excel') {
  ...
  for (const tx of report.transactions) {
    ...
    for (const item of tx.items) {
      entryRows.push([
        cpName, tx.invoice_number??'—', tx.register_entry_number??'—',
        item.product?.name??`ID:${item.product_id}`,
        parseFloat(item.purchase_no_tax), parseFloat(item.purchase_tax_amount), parseFloat(item.total_purchase),
        parseFloat(item.total_resale), parseFloat(item.resale_vat), parseFloat(item.resale_no_tax), parseFloat(item.markup),
      ]);
    }
  }
  ...
  for (const ex of report.exits) {
    ...
    for (const item of ex.items) {
      exitRows.push([cpName, ex.document_number??'—', item.product?.name??`ID:${item.product_id}`,
        parseFloat(item.total_sale), parseFloat(item.vat_amount), parseFloat(item.total_sale_no_vat)]);
    }
  }
  ...
  if (format === 'pdf') {
    ...
    doc.text(`${company.name} — ${dateLabel}`, 14, 16);
    doc.setFontSize(10); doc.setFont('helvetica','bold'); doc.setTextColor(60);
    doc.text('INTRĂRI', 14, 26); doc.setTextColor(0);
    autoTable(doc, { startY:30, head:[entryHeaders], body:entryRows.map(r=>r.map(String)), headStyles:{fillColor:[26,29,38],textColor:[232,236,244],fontSize:8,fontStyle:'bold'}, bodyStyles:{fontSize:8}, alternateRowStyles:{fillColor:[244,245,247]}, margin:{left:14,right:14} });
    const afterEntries = (doc as any).lastAutoTable.finalY + 8;
    doc.setFontSize(10); doc.setFont('helvetica','bold'); doc.setTextColor(60);
    doc.text('IEȘIRI', 14, afterEntries); doc.setTextColor(0);
    autoTable(doc, { startY:afterEntries+4, head:[exitHeaders], body:exitRows.map(r=>r.map(String)), headStyles:{fillColor:[26,29,38],textColor:[232,236,244],fontSize:8,fontStyle:'bold'}, bodyStyles:{fontSize:8}, alternateRowStyles:{fillColor:[244,245,247]}, margin:{left:14,right:14} });
