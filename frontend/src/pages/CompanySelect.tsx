...
export default function CompanySelect({ onSelect }: Props) {
  ...
  const openCreate = () => {
    setForm({ name: '', tax_id: '', chamber_id: '' });
    setStock(emptyStock()); setError(''); setTarget(null); setModalMode('create');
  };
  ...
  const openDelete = (c: Company, e: React.MouseEvent) => {
    e.stopPropagation(); setTarget(c); setError(''); setModalMode('delete');
  };
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setCompanies(p => [...p, c]); close(); onSelect(c);
  ...
  const handleEdit = async () => {
    ...
    try {
      const res = await fetch(`${BASE}/companies/${target.id}`, {
          ...
          <div className={styles.modal}>
            ...
            <div className={styles.stockRow}>
              <div className={styles.stockField}>
                <span className={styles.stockFieldLabel}>Fără TVA</span>
                <input className={`${styles.input} ${styles.mono}`} type="number" min="0" step="0.01" value={stock.no_vat} onChange={e => sf('no_vat', e.target.value)} placeholder="0.00" />
              </div>
              <div className={styles.stockField}>
                <span className={styles.stockFieldLabel}>TVA</span>
                <input className={`${styles.input} ${styles.mono}`} type="number" min="0" step="0.01" value={stock.vat} onChange={e => sf('vat', e.target.value)} placeholder="0.00" />
              </div>
              <div className={styles.stockField}>
                <span className={styles.stockFieldLabel}>Total</span>
                <input className={`${styles.input} ${styles.mono}`} type="number" min="0" step="0.01" value={stock.total} onChange={e => sf('total', e.target.value)} placeholder="0.00" />
              </div>
            </div>
            {error && <p className={styles.error}>{error}</p>}
            <div className={styles.modalActions}>
              <button className={styles.btnCancel} onClick={close}>Anulare</button>
              <button className={styles.btnConfirm} onClick={modalMode === 'create' ? handleCreate : handleEdit}>
